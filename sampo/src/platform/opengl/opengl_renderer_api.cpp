...
namespace Sampo
{
	...
	void OpenGLRendererAPI::DrawIndexed(const std::shared_ptr<VertexArray>& aVertexArray, uint32 anIndexCount)
	{
		const uint32 count = anIndexCount > 0 ? anIndexCount : aVertexArray->GetIndexBuffer()->GetCount();
